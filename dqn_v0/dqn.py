...
class DQN(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            #replay_data.reward 
            #replay_data.next_obs
            #replay_data.obs
            with th.no_grad():
               
                V = self.q_net_target(replay_data.next_obs)

                # 1-step TD target
                target_q_values = replay_data.reward + self.gamma * V

            # Get current Q-values estimates
            current_q_values = self.q_net(replay_data.obs)
