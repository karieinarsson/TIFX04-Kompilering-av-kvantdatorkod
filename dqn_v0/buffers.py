...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.obs = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=observation_space.dtype)

        if optimize_memory_usage:
            # `observations` contains also the next observation
            self.next_obs = None
        else:
            self.next_obs = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=observation_space.dtype)

        self.reward = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
...
#TODO
    def add(
        ...
        self.reward[self.pos] = np.array(reward).copy()

        if self.optimize_memory_usage:
            self.obs[(self.pos + 1) % self.buffer_size] = np.array(next_obs).copy()
        else:
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        if self.optimize_memory_usage:
            next_obs = self._normalize_obs(self.obs[(batch_inds + 1) % self.buffer_size, env_indices, :], env)
        else:
            next_obs = self._normalize_obs(self.next_obs[batch_inds, env_indices, :], env)

        data = (
            self._normalize_obs(self.obs[batch_inds, env_indices, :], env),
            next_obs,
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
            self._normalize_reward(self.reward[batch_inds, env_indices].reshape(-1, 1), env),
        )
