...
class swap_environment(Env):
    def __init__(self, depth: int, rows: int, cols: int, 
            max_swaps_per_time_step: int = -1, timeout: int = 200, gates: int = None) -> None:
        self.depth = depth
        self.rows = rows
        self.cols = cols
        ...
        if gates is None:
            self.gates = int(self.max_swaps_per_time_step * self.depth)
        else:
            self.gates = gates
        ...
        self.observation_space = Box(low=0, high=self.max_swaps_per_time_step,
                                shape=(1, depth, rows, cols, ), dtype=np.float)
    ...
    def step(self, action: int) -> Tuple[List[int], int, bool, 'info']:
        ...
        if self.max_episode_steps <= 0 or len(self.code) == 0:
            done = True
        else:
            done = False
    ...
    def reset(self, code: State = None) -> State:
        ...
        d, r, c = code.shape
        self.code = self.processing(code.reshape((d, r*c)), preprocessing = True)
    ...
    def get_code(self):
        return np.concatenate((self.state.reshape((self.depth, self.rows*self.cols)), self.code), axis=0)
    ...
    def make_state_slice(self, max_gates = np.inf) -> FlattenedTimeStepLayer:
        """
        Output: Flattened timestep layer of random gates
        """
        max_gates = math.floor(self.rows*self.cols/2) if math.floor(self.rows*self.cols/2)<max_gates else max_gates
        state_slice = np.zeros(self.rows*self.cols)
        x = np.random.choice(range(2, max_gates+2))
        for i in range(1, x):
            state_slice[i-1] = i
            state_slice[i-1+max_gates] = i
        np.random.shuffle(state_slice)
        return state_slice, x

    def make_code(self) -> State:
        """
        Output: State composed of random timestep layers with random gates
        """
        state = np.zeros((int(self.gates/self.max_swaps_per_time_step), self.rows, self.cols))
        added_gates = 0
        i = 0
        while added_gates < self.gates:
            s, gates_used = self.make_state_slice(self.gates-added_gates)
            state[i] = s.reshape((self.rows, self.cols))
            added_gates += gates_used
            i += 1
        return state
    ...
    def processing(self, state: State, preprocessing: bool = True) -> State:
        ...
        if preprocessing:
            padding = self.depth-return_state.shape[0] if self.depth>return_state.shape[0] else 0
            return_state = np.pad(return_state, ((0,padding),(0,0)))
        return return_state
